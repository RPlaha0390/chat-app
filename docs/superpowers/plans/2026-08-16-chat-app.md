...
- Modify: `server/src/app.js` (mount `/api/conversations` and `/api/users`)
...
- Produces: `GET /api/users -> { users: [{ id, username, avatarUrl }] }` (excludes the caller) — Task 9's `NewConversationModal` needs a list of users to pick from when starting a DM or group; the design spec calls for that picker but never named the endpoint that feeds it, so this task adds it.
...
- [ ] **Step 6: Write `server/src/controllers/userController.js`**
...
- [ ] **Step 7: Write `server/src/routes/userRoutes.js`**
...
- [ ] **Step 8: Mount both route modules in `server/src/app.js`**
...
```bash
git add server/
git commit -m "Add conversations, messages, and user-listing REST API with membership checks"
...
```bash
gh pr create --title "Conversations & messages API" --body "REST endpoints for creating DMs/groups, listing a user's conversations, sending a message, paginated history, and listing other users (for the new-conversation picker). createMessage/assertMembership are exported standalone so the Socket.IO layer (next task) reuses this exact logic instead of duplicating it."
```
...
- [ ] **Step 4: Write `client/src/api/conversations.js` and `client/src/api/users.js`**
...
export function ChatPage() {
  ...
  useEffect(() => {
    listUsers().then((data) => setUserOptions(data.users));
  }, []);
